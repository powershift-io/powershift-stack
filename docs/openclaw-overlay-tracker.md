...
|---|---|---|---|---|
| Governed Collaboration Adapter for Buzz and OpenClaw, protocol `0.1` | **Supervised reference deployment proven** | **Implemented as release candidate** | `overlays/openclaw/buzz-governed-agency/` | Public-safe TypeScript governance membrane with signed intake, canonical actor binding, authority evaluation, role projection, authorized context, exact-session dispatch, replay control, operator lifecycle controls, durable single-supervisor state, and receipt return. Broader unattended and horizontally scaled operation remains held. |
...
| OpenClaw backup and restore strategy | **Used in reference operations** | **Implemented as working guide/templates** | `docs/orchestrator-backup-restore-strategy.md`, `templates/pre-change-snapshot-checklist.md`, `templates/backup-retention-policy.md`, `templates/backup-freshness-receipt.md`, `templates/monthly-restore-drill.md` | Layered safety model for hot state, cold corpus, versioned identity, offsite recovery, retention policy, freshness evidence, and restore drills. |
